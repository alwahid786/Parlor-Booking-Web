                                            ...
                                            <div class=" mb-3 for_appointments_card_width">
                                                <div class="row g-0">
                                                    ...
                                                    <div class="col-xxl-8 col-xl-8 col-lg-8 col-md-8 col-sm-8 col-8">
                                                        {{--  @if ($past_appointment->status == 'completed')  --}}
                                                            <div class="card-body card_body_for_appointment {{ $past_appointment->status == 'completed' ? 'for_complete_css' : ''  }}">
                                                                <button type="button" class="btn btn-sm {{ $past_appointment->status == 'completed' ? '' : 'px-4' }} "> {{ $past_appointment->status == 'completed' ? 'Completed' : 'Cancel'  }}</button>
                                                        {{--  @else  --}}
                                                                {{--  <button type="button" class="btn btn-sm px-4">Cancel</button>  --}}
                                                        {{--  @endif  --}}
                                                            <h5>{{ $past_appointment->user->name }}</h5>
                                                            @foreach ($past_appointment->appointment_details as $appoint_details)
                                                                <h6>{{ $appoint_details->services->name }}</h6>
                                                            @endforeach
                                                            <p> {{  date('d/m/Y', strtotime($past_appointment->date)) }} {{ date('h:i A', strtotime($past_appointment->start_time)) ?? ''}}</p>
                                                            <h4>${{ $past_appointment->total_price }}</h4>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
